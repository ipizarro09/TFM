...
function App() {
  ...
  useEffect(() => {
    if (dataset) {
      ...
      } else if (numRecords >= 101 && numRecords <= 10000) {
  ...
  return (
    <div className="App">
      ...
      <h4>Data collected for recommendation</h4>
        <p><strong>Purpose of Visualization:</strong> {proposito || "Not selected"}</p>
        <p><strong>Context of Visualization:</strong> {contexto || "Not selected"}</p>
        ...
        <p><strong>Number of dimensions selected:</strong> {nDimensiones || "Not found"}</p>
		    <p><strong>In case of numerical, is any variable ordered?:</strong> {ordenadas || "Not selected"}</p>
		    <p><strong>In case of categorical, what is the relation?:</strong> {relacion || "Not selected"}</p>
        <p><strong>High number of groups in data:</strong> {nGruposAlto || "Not selected"}</p>
        <p><strong>Observations per group:</strong> {obsGrupo || "Not selected"}</p>
      </div>
      <button
        onClick={() =>
          sendData({
...
/*function sendData(data) {
  ...
  fetch("http://localhost:3001/submit-form", requestOptions)
